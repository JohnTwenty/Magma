...
#pragma warning( disable : 4710) // 'int sprintf_s<32>(char (&)[32],const char *,...) throw()' : function not inlined
...
bool ODBlock::loadScript(const char* buffer, unsigned size)			//loads block from script, including subbocks
{
	return (parse(buffer, size) != NULL);
}

const char* ODBlock::parse(const char* buffer, unsigned size)			//loads block from script, including subbocks
{
	int c;
	...
	while(buffer < end)
	{
		...
		while (ignoreWhiteSpace && ((c == ' ') || (c == '\t') || isNewLine((char)c)) && (buffer < end));
		...
		if  (c == '"')
		{
			if (state == BLOCK)
			{
				...
				addStatement(*child);
				buffer = child->parse(buffer, static_cast<unsigned>(end - buffer));
				if (!buffer)
					return buffer;
			}
			else
		...
		else
		{
			...
			else
			{
				...
				{
				case '#':
					while(!isNewLine(*buffer++)) ; //read and discard line
					break;
				case '/':
					while(*buffer++ != '/') ; //read and discard comment
					break;
				case '{': 
...
bool ODBlock::saveScript(FILE * fp,bool bQuote)
{
	...
	if (!bTerminal)
	{
		...
		retVal = fprintf(fp,"{\n");
		tablevel ++;		
		for (MxU32 i = 0; i < subBlocks.size(); ++i)
...
const char * ODBlock::nextTerminal()								//returns a pointer to the next terminal string.  
{
...
int ODBlock::nextTerminalAsInt()
	{
	int p = -1;
	sscanf_s(nextTerminal(), "%d", &p);
	return p;
	}

float ODBlock::nextTerminalAsFloat()
	{
	float p = 0.0f;
	sscanf_s(nextTerminal(), "%f", &p);
	return p;
	}
bool ODBlock::nextTerminalAsBool()
	{
	return nextTerminalAsInt() != 0;
	}
...
bool ODBlock::getBlockInt(const char * ident, int* p, unsigned count)		//reads blocks of form:		ident{ 123; 123; ... }
{
	...
	if (temp)
	{
		...
		for(; count && temp->moreTerminals(); --count)
			if(p)	
				sscanf_s(temp->nextTerminal(),"%d", p++);
...
// hig level macro functs, return true on success:
bool ODBlock::getBlockU32(const char * ident, MxU32* p, unsigned count)		//reads blocks of form:		ident{ 123;}
{
	ODBlock* temp = getBlock(ident);
	if (temp)
	{
...
bool ODBlock::getBlockFloat(const char *  ident, float *p)		//of form:				ident{123.456;}
{
	...
	if (temp)
	{
		...
		if (temp->moreTerminals())	
		{ 
			if(p) sscanf_s(temp->nextTerminal(),"%f",p); 
			return true; 
		}
	}
	return false;
...
bool ODBlock::getBlockFloats(const char *  ident, float *p, unsigned numfloats)//form:		ident{12.3; 12.3; 12.3; ... };
{
	...
	if (temp)
	{
		...
		for (unsigned int n=0; n<numfloats; n++)
			if (temp->moreTerminals())	sscanf_s(temp->nextTerminal(),"%f",&(p[n]));
